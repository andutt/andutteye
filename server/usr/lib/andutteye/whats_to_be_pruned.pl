...
for(@tables) {
	my $table = $_;
	print "Will prune data older then $prune_after_date in table->$table.";
	$sth = $dbh->prepare("select count(seqnr) from $table where created_date < '$prune_after_date'");
	$sth->execute;
	my $row = $sth->fetchrow_array;
        print "\twill delete = $row rows\n";
}
